...
if [ -n "$GITHUB_TOKEN" ]; then
    ...
    if [ -n "$RELEASE_ID" ] && [ -n "$UPLOAD_URL" ]; then
        ...
        # Upload Windows assets (Setup.exe, nupkg, releases.win.json, assets.win.json, RELEASES)
        SETUP_EXE=$(find "$RELEASES_PATH" -name "*-Setup.exe" | head -1)
        if [ -f "$SETUP_EXE" ]; then
            upload_asset "$SETUP_EXE" "application/octet-stream"
        fi
        
        NUPKG_FILE=$(find "$RELEASES_PATH" -name "*.nupkg" | head -1)
        if [ -f "$NUPKG_FILE" ]; then
            upload_asset "$NUPKG_FILE" "application/octet-stream"
        fi
