...
namespace IBudget.GUI.ViewModels
{
    public partial class DashboardViewModel : ViewModelBase
    {
        ...
        [ObservableProperty]
        private string _thisMonthString = string.Empty;
        ...
        private readonly string[] _fullMonthNames = new[]
        {
            "January", "February", "March", "April", "May", "June",
            "July", "August", "September", "October", "November", "December"
        };
        [ObservableProperty]
        private double[] _previousMonthsValues = new double[] { 2, 5, 4, 10, 7 };
        [ObservableProperty]
        private string[] _labels = new string[] { "November", "December", "January", "February", "March" };

        public DashboardViewModel(ITagService tagService, ISummaryService summaryService, IMessageService messageService, IFinancialGoalService financialGoalService)
        {
            ThisMonth = DateTime.Now.ToString("MMMM");
            ThisMonthString = $"{ThisMonth} {DateTime.Now:yyyy}";
            ...
            LastMonthLabel = $"< {_monthNames[(thisMonthInt + 10) % 12]}"; // Previous month
            NextMonthLabel = $"{_monthNames[thisMonthInt % 12]} >"; // Next month
        ...
        partial void OnSelectedIndexChanged(int value)
        {
            ...
            try
            {
                if (value >= 0 && value < _fullMonthNames.Length)
                {
                    ThisMonth = _fullMonthNames[value];
        ...
        {
            ThisMonth = _fullMonthNames[(SelectedIndex + 1) % 12];
            ThisMonthString = $"{ThisMonth} {DateTime.Now:yyyy}";
        ...
        {
            ThisMonth = _fullMonthNames[(SelectedIndex - 1 + 12) % 12];
            ThisMonthString = $"{ThisMonth} {DateTime.Now:yyyy}";
            SelectedIndex = (SelectedIndex - 1) % 12;
            var thisMonthInt = DateTime.ParseExact(ThisMonth, "MMMM", CultureInfo.InvariantCulture, DateTimeStyles.None).Month;
            LastMonthLabel = $"< {_monthNames[(thisMonthInt + 10) % 12]}"; // Previous month
